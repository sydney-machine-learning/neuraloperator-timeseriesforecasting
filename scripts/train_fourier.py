...
from model.ml import SpectralConv2d
...
def evaluate_preds(model, ds, batch_size, y_mu, y_sigma, q_in):
...
def train_and_evaluate(train_ds, val_ds,
                       ...
                       run_dir='/project/results/fourier',
                       **kwargs):
    ...
    X_train, y_train = create_sequence(X_train, y_train,
                                       ...
                                       q_in=kwargs['q_in'])
    X_val, y_val = create_sequence(X_val, y_val,
                                   window_size=kwargs['window_size'],
                                   q_in=kwargs['q_in'])
    ...
    # Create SpectralConv model
    model = SpectralConv2d(in_channels=1, out_channels=1, modes1=3, modes2=4)
    ...
    # Evaluate on train data
    nse_train, nnse_train, fig_train = evaluate_preds(model, train_ds, batch_size, y_mu, y_sigma, q_in=kwargs['q_in'])
    ...
    nse_val, nnse_val, fig_val = evaluate_preds(model, val_ds, batch_size, y_mu, y_sigma, q_in=kwargs['q_in'])
...
if __name__ == '__main__':
    ...
    if args.station_id is None:
        # station ids change here for no of stations.
